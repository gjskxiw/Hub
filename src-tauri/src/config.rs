...
impl Config {
    ...
    ///
    /// 为什么必须做：
    /// - 悬空的 envId 会让工具处于「看着绑定了某个 Java 环境，实际无法启动」的状态，
...
fn window_path(data_dir: &Path) -> PathBuf {
...
/// 数据目录固定为 exe 同目录的 data\。
///
/// 只在本机使用，不假设应用会被搬到不可写的介质上，所以没有「先探测目录可否写入、
/// 不可写就改到 %APPDATA%」这一层：目录真的不能写时，让保存配置直接报错更好排查，
/// 而不是悄悄换个地方写、让人以为配置丢了。
pub fn resolve_data_dir() -> PathBuf {
    let exe = std::env::current_exe().expect("无法确定程序自身的位置");
    let data = exe
        .parent()
        .expect("程序路径没有上级目录")
        .join("data");
...
mod tests {
    ...
    fn unknown_enum_values_are_tolerated() {
        // terminal 与 version 都是已删除的字段（version 从来没被任何逻辑读过），
        // 出现在旧文件里应作为未知字段被静默忽略，而不是让整份配置解析失败
        let json = r#"{
            "version": 1,
