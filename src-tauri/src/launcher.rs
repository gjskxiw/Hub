...
fn bound_env<'a>(tool: &Tool, config: &'a Config) -> Option<&'a Env> {
...
/// Java：env.path 为 Java 安装根目录；Python：env.path 为 python.exe 所在目录。
...
mod tests {
    ...
    fn validate_env_error_prefixes_are_stable() {
        ...
        let missing = validate_env(&env(EnvKind::Python, "C:\\ovo-self-test\\nope")).unwrap_err();
    ...
    fn unbound_env_is_rejected() {
        ...
        let dir = std::env::temp_dir().join(format!("ovo-argv-{}", std::process::id()));
        ...
        // 已绑定环境（即便指向的 Java 安装目录并不存在）报的是「Java 不存在」，而不是未绑定
