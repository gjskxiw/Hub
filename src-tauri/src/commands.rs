...
#[serde(rename_all = "camelCase")]
pub struct EnvProbe {
    /// 建议的环境名（由版本信息推导，如 "Java 17.0.9" / "Python 3.12.4"）
    pub name: String,
}
...
/// 按界面主题更换窗口/任务栏图标与托盘图标。
///
/// 打包进 exe 的那张跟不了主题（Windows 只认 .ico 里的层），所以这里只换运行时可见的两处。
/// 失败只落 app.log：图标没换成不该挡住主题切换本身。
#[tauri::command]
pub async fn set_app_icon(
    ...
    let image =
        tauri::image::Image::from_bytes(bytes).map_err(|e| format!("解析应用图标失败: {e}"))?;
    let data_dir = state.data_dir.clone();
    // 动的是原生图标句柄，必须回主线程，与托盘重建同一套做法
    let app_inner = app.clone();
    app.run_on_main_thread(move || {
        if let Some(window) = app_inner.get_webview_window("main") {
            if let Err(e) = window.set_icon(image.clone()) {
                logging::error(&data_dir, &format!("更换窗口图标失败: {e}"));
            }
        }
        if let Some(tray) = app_inner.tray_by_id(tray::TRAY_ID) {
            if let Err(e) = tray.set_icon(Some(image)) {
                logging::error(&data_dir, &format!("更换托盘图标失败: {e}"));
            }
        }
    })
    .map_err(|e| format!("调度图标更换失败: {e}"))
}
