...
// 同一图形的两个色版：light.png 是纯白（只有深底看得见），dark.png 是近黑（只有浅底看得见），
// 所以与主题是交叉对应：浅色主题用 dark.png，深色主题用 light.png。
import logoBlack from "../dark.png";
import logoWhite from "../light.png";
...
<template>
  <TooltipProvider :delay-duration="300">
    ...
    <div v-else class="flex h-screen flex-col overflow-hidden">
      ...
      <TitleBar>
        <div class="flex shrink-0 items-center pl-3">
          <img
            :src="store.config.settings.theme === 'dark' ? logoWhite : logoBlack"
            class="size-5"
            alt="ovo"
          />
        </div>
