...
Windows 上的本机工具启动面板：把散在各处的脚本、JAR、绿色程序、工具页收成一张张卡片，双击即启动。
...
| | |
| --- | --- |
| 六种工具类型 | Terminal / Gui 各自的 Python、Java、exe，外加 Web 链接 |
| 运行环境 | 选一次安装目录即可，程序跑版本命令自动命名；Java / Python 工具必须绑定环境，不回退系统 `PATH` |
| 分组与搜索 | 侧栏按分组筛选、可上色标；搜索匹配名称与描述 |
| 托盘 | 按分组列出工具，不开主窗口也能启动 |
| 提权启动 | Gui exe 支持右键「以管理员身份运行」，一次性动作 |
| 打开工作目录 | 右键直达目标文件所在目录 |
| 配置导入导出 | 一个 JSON 文件；导入前校验路径与引用，替换前二次确认 |
| 深浅主题 | 标题栏与应用图标随主题切换色版 |
| 数据安全 | 原子写 + 滚动备份，主文件损坏时自动从备份恢复 |
| 窗口记忆 | 恢复上次尺寸与最大化状态 |

## 使用方法

1. **下载运行**：Releases 里取 `ovo.exe`，放到任意目录双击。首次运行 Windows 可能弹 SmartScreen 提示——没有代码签名，属正常。
2. **加运行环境**（只有 Java / Python 类工具需要）：点顶栏的运行环境图标 → 添加 → 选目录。Java 选安装根目录，Python 选 `python.exe` 所在目录。
...
配置与图标都写在 exe 旁边的 `data\` 目录里；运行日志在 `data\logs\app.log`，托盘菜单可直接打开。

## 构建

```bash
pnpm install
pnpm tauri dev      # 开发（Vite + 热重载）
pnpm tauri build    # 出单文件 exe
cd src-tauri && cargo test
```

仅支持 Windows，无自动更新。许可见 [LICENSE](LICENSE)。
